...
public class Plugin extends Application implements Runnable {
	...
	private Update u; // Objekt der Update-Klasse (Lässte ein Fenster
						// erscheinen, sobald eine neuere Version verfügbar ist)
	private Fenster f;

	private String host = "192.168.1.25"; // Die Ip des Rechnsers, auf welchem
											// die Sim läuft (Wird bei einer
	...
	public void start(Stage primaryStage) {
		Runnable r = () -> {
			...
			// p.widthProperty().addListener((observable, oldValue, newValue) ->
			// tfHost.setTranslateX(newValue.doubleValue()/2 - 75));
			...
			Platform.runLater(() -> {
				try {
					primaryStage.getIcons().add(new Image(
							Plugin.class.getResourceAsStream("res/icon.png")));
				} catch (Exception e) {
					try {
			...
			try {
				final java.util.Enumeration<NetworkInterface> nics = NetworkInterface
						.getNetworkInterfaces();
				while (socketGefunden == null && nics.hasMoreElements()) {
					java.net.NetworkInterface nic = nics.nextElement();
					final java.util.Enumeration<InetAddress> inetAddresses = nic
							.getInetAddresses();
					while (socketGefunden == null
							&& inetAddresses.hasMoreElements()) {
						final InetAddress inetAddress = inetAddresses
								.nextElement();
						final Socket socket = new Socket();
						try {
							socket.bind(null);
							socket.connect(
									new InetSocketAddress(inetAddress, 3691));
							if (socketGefunden == null || cmpInetAddress(
									socket.getLocalAddress(),
									socketGefunden.getLocalAddress()) < 0) {
								socket.setSoTimeout(1000);
								socketGefunden = socket;
								Platform.runLater(() -> tfHost
				...
				Platform.runLater(() -> {
					if (tfHost.getText().equals("Suche IP-Adressen..."))
	...
	/**
	 * Vergleicht zwei IP Adressen. * Link local < local < unique < multicast *
	 * loopback < IPv4 < IPv6
	 *
	 * @param address0
	 * @param address1
	 ...
	 * @return
	 */
	private static int cmpInetAddress(final InetAddress address0,
			final InetAddress address1) {
		...
		// IPv4 < IPv6
		if (InetAddress.class.isAssignableFrom(address0.getClass())
				^ InetAddress.class.isAssignableFrom(address1.getClass())) {
			return InetAddress.class.isAssignableFrom(address0.getClass())
					? -1
					: 1;
		}
		return 0;
	...
	void errorWindow(int exitCode, String message) {
		Platform.runLater(() -> {
			...
			Label fehlerCode = new Label(
					"Diese Zahl kann bei der Fehlersuche helfen: " + exitCode);
	...
	private void setOutputStreams() {
		try {
			...
			if (f.exists() && f.canRead()) {
				System.out.println(
						"Fehler und Meldungen werden in log-Datei geschrieben! Speicherort: "
								+ f.getAbsolutePath());
	...
	private void checkLogOnClosing() {
		try {
			...
			while (temp != null && !temp.contains("java")
					&& !temp.contains("com.") && !temp.contains("GITHUB")) {
				temp = raf.readLine();
			}

			if (temp != null && (temp.contains("java") || temp.contains("com.")
					|| temp.contains("GITHUB"))) {
	...
	private void openLogWindow() {
		...
		// Label l = new Label("Während deiner aktuellen Sitzung sind Fehler
		// aufgetreten. Durch einen Klick auf Weiter werden deine Log-Datei und
		// deine Anregungen anonym hochgeladen.");
		Label l = new Label(
		 ...
		 * ta.setFont(Font.font(settingsFontSize-3)); ta.setTranslateX(25);
		 * ta.setTranslateY(125); ta.setPrefWidth(450); ta.setPrefHeight(100);
		 */
	...
	private void sendLogFile(Label l) {
		try {
			...
			// String baseUrl =
			// "http://localhost/Webseiten/Gleisbelegung/request.php";
			...
			for (int i = 0; i < log.length(); i++) {
				...
				if (logArray.get(counter).length() >= 2000 - 10) { // -10 wegen
																	// versionsnummer
					logArray.set(counter,
							logArray.get(counter).replace(" ", "%20"));
					logArray.set(counter,
							logArray.get(counter).replace("\n", "%0A"));

					counter++;
					logArray.add(counter, "");
				}
			}
			logArray.set(counter, logArray.get(counter).replace(" ", "%20")); // für
																				// das
																				// letzte
																				// element
			...
			URL url = new URL(
					baseUrl + "?action=new&message=v." + version + "&log="
							+ URLEncoder.encode(logArray.get(0),
			...
			while (counter < logArray.size()) {
				try {
					...
					Platform.runLater(() -> l.setText("Lade " + temp + " von "
							+ (logArray.size() - 1) + " hoch..."));

					url = new URL(baseUrl + "?action=add&id=" + id + "&log="
							+ URLEncoder.encode(logArray.get(counter),
			...
			Platform.runLater(() -> l.setText(
					"Fertig. Vielen Dank für deine Hilfe!\nDeine Log-Id ist "
							+ id));
			Runnable r = () -> {
				try {
					Thread.sleep(5000);
		...
		while (update) {
			try {
				...
				if (time >= Einstellungen.update) {
					...
					synchronized (updateMonitor) {
						if (updateThread.isUpdating()) {
							System.err.println("Update ausgefallen");
						}
