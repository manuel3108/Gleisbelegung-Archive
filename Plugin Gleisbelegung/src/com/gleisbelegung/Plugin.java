...
public class Plugin extends Application implements Runnable {
    ...
    private Stage primaryStage;
            //Erste Benutzeroberfläche, die beim start angezeigt wird.
    private StackPane firstSP;
    private Update u;
            //Objekt der Update-Klasse                                  (Lässte ein Fenster erscheinen, sobald eine neuere Version verfügbar ist)
    private Fenster f;
    private String host = "192.168.1.25";
            //Aktualle Version des Plugins
            //Die Ip des Rechnsers, auf welchem die Sim läuft           (Wird bei einer Änderung beim Pluginstart aktualisiert)
    ...
    private void startLoading(Socket socketGefunden) {
        Runnable r = () -> {
            ...
            try {
                if (stellwerk != null && stellwerk.getSocket() != null)
                    stellwerk = new Stellwerk(stellwerk.getSocket(),
                            "Gleisbelegung", "Darstellung der Gleisbelegung",
                            "Manuel Serret", version);
                else if (socketGefunden != null)
                    stellwerk = new Stellwerk(socketGefunden, "Gleisbelegung",
                            "Darstellung der Gleisbelegung", "Manuel Serret",
                            version);
                else
                    stellwerk = new Stellwerk(host, 3691, "Gleisbelegung",
                            "Darstellung der Gleisbelegung", "Manuel Serret",
                            version);
    ...
    private void sendLogFile(Label l) {
        try {
            ...
            for (int i = 0; i < log.length(); i++) {
                ...
                if (logArray.get(counter).length()
                        >= 2000 - 10) { //-10 wegen versionsnummer
            ...
            logArray.set(counter, logArray.get(counter)
                    .replace(" ", "%20"));        //für das letzte element
