...
public class Plugin extends Application implements Runnable {
    ...
    @Override public void start(Stage primaryStage) {
        Runnable r = () -> {
            ...
            try {
                final java.util.Enumeration<NetworkInterface> nics =
                        NetworkInterface.getNetworkInterfaces();
                while (socketGefunden == null && nics.hasMoreElements()) {
                    java.net.NetworkInterface nic = nics.nextElement();
                    final java.util.Enumeration<InetAddress> inetAddresses =
                            nic.getInetAddresses();
                    while (socketGefunden == null && inetAddresses
                            .hasMoreElements()) {
                        ...
                        try {
                            ...
                            if (socketGefunden == null ||
                                    cmpInetAddress(socket.getLocalAddress(),
                                            socketGefunden.getLocalAddress())
                                            < 0) {
    ...
    private void checkLogOnClosing() {
        try {
            ...
            while (temp != null && !temp.contains("java") && !temp
                    .contains("com.") && !temp.contains("GITHUB")) {
                temp = raf.readLine();
            }
    ...
    private void sendLogFile(Label l) {
        try {
            ...
            String baseUrl =
                    "http://manuel-serret.bplaced.net/Gleisbelegung/request.php";
            // String baseUrl =
            ...
            logArray.set(counter,
                    logArray.get(counter).replace(" ", "%20")); // für
            // das
            // letzte
            // element
            logArray.set(counter, logArray.get(counter).replace("\n", "%0A"));

            URL url = new URL(baseUrl + "?action=new&message=v." + version
                    + "&log=" + URLEncoder.encode(logArray.get(0),
