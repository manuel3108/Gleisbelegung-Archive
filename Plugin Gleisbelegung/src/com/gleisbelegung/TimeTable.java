...
public class TimeTable {
    ...
    public void addFahrplanHalt(FahrplanHalt fh) {
        ...
        for (TimeTableRow ttr : rows) {
            if (ttr.time >= fh.getTatsaechlicheAnkunft()
                    && ttr.time <= fh.getTatsaechlicheAbfahrt() + 1000 * 60) {
        ...
        int haltInMinuten = (int) ((fh.getTatsaechlicheAbfahrt() - fh
                .getTatsaechlicheAnkunft()) / (1000 * 60));
        if (haltInMinuten > counter && rows.size() > 0) {
            TimeTableRow lastRow = rows.get(rows.size() - 1);
            while (fh.getAbfahrt() + fh.getZug().getVerspaetungInMiliSekunden()
                    > lastRow.time) {
                ...
                if (lastRow.time >= fh.getTatsaechlicheAnkunft() && lastRow.time
