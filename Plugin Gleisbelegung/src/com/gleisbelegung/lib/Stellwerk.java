...
public class Stellwerk {
    ...
    public Stellwerk(String host, int port, String pluginName,
            String pluginBeschreibung, String autor, int version)
            throws IOException {
        this.host = host;
        this.port = port;
        ...
        v = new Verbindung(new Socket(host, port), this, pluginName,
                pluginBeschreibung, autor, version);
        v.update();
    }

    public Stellwerk(Socket socket, String pluginName,
            String pluginBeschreibung, String autor, int version) {
        host = null;
        port = 0;
        ...
        v = new Verbindung(socket, this, pluginName, pluginBeschreibung, autor,
                version);
        v.update();
    }
    ...
    void erstelleBahnhoefe(String[] bahnsteige) {
        ...
        for (int i = 0; i < bahnsteige.length; i++) {
            ...
            if (letzterBahnhofsName.equals(bahnhofsName[i])
                    && bahnhoefe.size() > 0) {
                bahnhoefe.get(bahnhoefe.size() - 1).addBahnsteig(
                        new Bahnsteig(bahnhoefe.get(bahnhoefe.size() - 1),
                                bahnsteigsName, i));
            } else {
                letzterBahnhofsName = bahnhofsName[i];
                bahnhoefe.add(new Bahnhof(bahnhoefe.size(),
                        letzterBahnhofsName));
                bahnhoefe.get(bahnhoefe.size() - 1).addBahnsteig(
                        new Bahnsteig(bahnhoefe.get(bahnhoefe.size() - 1),
                                bahnsteigsName, i));
    ...
    @Override public String toString() {
        return "Stellwerk{" + "stellwerksname='" + stellwerksname + '\''
                + ", anlagenid=" + anlagenid + ", simbuild=" + simbuild
                + ", startzeit=" + startzeit + ", spielzeit=" + spielzeit
