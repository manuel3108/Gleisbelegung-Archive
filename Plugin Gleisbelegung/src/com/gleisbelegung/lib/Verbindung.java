...
public class Verbindung {
    ...
    Verbindung(Socket socket, Stellwerk stellwerk, String pluginName,
        ...
        for (XML xmlZug : zugliste.getInternXML()) {
            try {
                ...
                for (Zug z : stellwerk.getZuege()) {
                    try {
                        if (!xmlZug.get("zid").equals("")
                                && Integer.parseInt(xmlZug.get("zid")) == z
                                .getZugId()) {
                            exists = true;
                            break;
                        }
                ...
                if (!exists && !xmlZug.get("zid").equals("") && !xmlZug
                        .get("name").equals("")) {
                    //System.out.println("INFORMATION: " + zugliste.get(i).get(0).get(1)[1] + " wurde hinzugefügt!");
                    stellwerk
                            .addZug(new Zug(Integer.parseInt(xmlZug.get("zid")),
                                    xmlZug.get("name")));
                }
            } catch (Exception e) {
                    ...
                    for (int i = 0; i < zugfahrplanEintraege.size(); i++) {
                        try {
                            ...
                            fahrplan.add(new FahrplanHalt(Long.parseLong(
                                    zugfahrplanEintrag.get("ab")),
                                    sucheBahnsteig(
                                            zugfahrplanEintrag.get("name")),
                                    zugfahrplanEintrag.get("flags"),
                                    sucheBahnsteig(
            ...
            try {
                if (z != null && z.getFahrplan() != null) {
                    for (FahrplanHalt fh : z.getFahrplan()) {
                        try {
                            if (fh != null && fh.getFlags() != null && !fh
                                    .getFlags().equals("")) {
                                Zug flagged = getFlaggedTrain(fh.getFlags());
                                if (flagged != null
                                        && flagged.getFahrplan() != null
                                        && flagged.getFahrplan(0) != null) {
                ...
                System.out
                        .println("ZUG: " + z.getZugName() + ": Flag-Problem!");
                e.printStackTrace();
            }
        }
    ...
    @Override public String toString() {
        return "Verbindung{" + "socket=" + socket + ", xml=" + xml
                + ", aktualisiere=" + aktualisiere + ", stellwerk=" + stellwerk
                + '}';
    }
}
